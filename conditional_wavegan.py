...
class ConditionalWaveGANGenerator(nn.Module):
    def __init__(self, n_classes=10, model_size=64, ngpus=1, num_channels=1,
                 latent_dim=100, post_proc_filt_len=512,
                 verbose=False, upsample=True):
        super(ConditionalWaveGANGenerator, self).__init__()
        self.ngpus = ngpus
        ...
        self.fc1 = nn.Linear(latent_dim + n_classes, 256 * model_size)
    ...
    def forward(self, x, y_ohe):
        x = torch.cat((x, y_ohe), dim=1)
...
class ConditionalWaveGANDiscriminator(nn.Module):
    def __init__(self, n_classes=10, model_size=64, ngpus=1, num_channels=1, shift_factor=2,
                 alpha=0.2, verbose=False):
        super(ConditionalWaveGANDiscriminator, self).__init__()
        ...
        self.fc1 = nn.Linear(256 * model_size, model_size)
        self.fc2 = nn.Linear(model_size + n_classes, 1)
    ...
    def forward(self, x, y_ohe):
        ...
        x = self.fc1(x)
        x = torch.cat((x, y_ohe), dim=1)  # concat label OHE vector to feature vector
        x = self.fc2(x) 
        return x
